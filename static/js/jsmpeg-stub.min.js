...
onmessage = function(ev) {
	switch(ev.data[0]) {
		case 0:
			{
				player = new JSMpeg.Player(ev.data[1], {
					dispatcher: self,
					video: true,
					videoBufferSize: 3145728
				});
				break
			}
		case 1:
		...
		case 3:
			...
			break;
		case 4:
			if(player) player.update(ev.data[1]);
			break;
		case 5:
			{
				var status = player.playoutStatus;postMessage([2, status.videoBytes, status.videoFrameCnt, status.videoDecodeFrameCnt, status.videoDecodeTime, status.videoBreakCnt]);
				break
			}
		case 6:
			{
				if(player) player.enableDecode(ev.data[1]);
				break
			}
		case 7:
			{
...
JSMpeg.Player = function() {
	...
	var Player = function(url, options) {
		...
		if(options.video !== false) {
			...
			this.renderer = new JSMpeg.Renderer.Stub(options);
			this.demuxer.connect(JSMpeg.Demuxer.TS.STREAM.VIDEO_1, this.video);
			this.video.connect(this.renderer)
		}
		Object.defineProperty(this, "playoutStatus", {
			get: this.getPlayoutStatus
		});
		this.videoBreakCnt = 0;
		this.source.start();
		if(this.autoplay) {
			this.play()
		}
	...
	Player.prototype.update = function(now) {
		...
		if(this.options.streaming) {
			this.updateForStreaming(now)
		}
	};
	Player.prototype.updateForStreaming = function(now) {
		var timestamp = now;
		if(this.video) {}
	};
	Player.prototype.getPlayoutStatus = function() {
		var status = {};
		status.videoBytes = this.demuxer.videoBytes;
		status.videoFrameCnt = this.demuxer.videoFrameCnt;
		status.videoDecodeFrameCnt = this.video.frameCnt;
		status.videoDecodeTime = this.video.resetCnt;
		status.videoBreakCnt = this.videoBreakCnt;
		return status
	};
	Player.prototype.enableDecode = function(enable) {
		this.video.setEnable(enable)
	};
	return Player
}();
...
JSMpeg.Demuxer.TS = function() {
	...
	var TS = function(options) {
		...
		this.startTime = 0;
		this.currentTime = 0;
		Object.defineProperty(this, "videoBytes", {
			get: this.getVideoBytes
		});
		Object.defineProperty(this, "videoFrameCnt", {
			get: this.getVideoFrameCnt
		});
		Object.defineProperty(this, "audioBytes", {
			get: this.getAudioBytes
		});
		Object.defineProperty(this, "audioFrameCnt", {
			get: this.getAudioFrameCnt
		})
	};
	...
	TS.prototype.destroy = function() {
		var pi = this.pesPacketInfo[TS.STREAM.VIDEO_1];
		if(pi.destination !== undefined) {
			pi.destination = null
		}
		pi = this.pesPacketInfo[TS.STREAM.AUDIO_1];
		if(pi.destination !== undefined) {
			pi.destination = null
		}
	};
	...
	TS.prototype.packetComplete = function(pi) {
		...
		pi.bytes = pi.bytes + pi.currentLength;
		pi.frameCnt++;
		pi.totalLength = 0;
		pi.currentLength = 0;
		pi.buffers = []
	};
	TS.prototype.getAudioBytes = function() {
		var pi = this.pesPacketInfo[TS.STREAM.AUDIO_1];
		return pi.bytes
	};
	TS.prototype.getAudioFrameCnt = function() {
		var pi = this.pesPacketInfo[TS.STREAM.AUDIO_1];
		return pi.frameCnt
	};
	TS.prototype.getVideoBytes = function() {
		var pi = this.pesPacketInfo[TS.STREAM.VIDEO_1];
		return pi.bytes
	};
	TS.prototype.getVideoFrameCnt = function() {
		var pi = this.pesPacketInfo[TS.STREAM.VIDEO_1];
		return pi.frameCnt
	};
	TS.STREAM = {
...
JSMpeg.Decoder.Base = function() {
	...
	var BaseDecoder = function(options) {
		...
		this.startTime = 0;
		this.decodedTime = 0;
		this.decodedFrameCnt = 0;
		this.decoderResetCnt = 0;
		Object.defineProperty(this, "currentTime", {
			get: this.getCurrentTime
		});
		Object.defineProperty(this, "frameCnt", {
			get: this.getDecodedFrameCnt
		})
	};
	...
	BaseDecoder.prototype.getDecodedFrameCnt = function() {
		return this.decodedFrameCnt
	};
	return BaseDecoder
}();
JSMpeg.Decoder.MPEG1Video = function() {
	...
	var MPEG1 = function(options) {
		...
		this._resetCnt = 0;
		Object.defineProperty(this, "resetCnt", {
			get: this.getResetCnt
		});
		this.enabled = true
	};
	...
	MPEG1.prototype.getResetCnt = function() {
		return this._resetCnt
	};
	MPEG1.prototype.setEnable = function(enabled) {
...
JSMpeg.Renderer.Stub = function() {
	"use strict";
	var StubRenderer = function(options) {
		this.dispatcher = options.dispatcher
	};
	StubRenderer.prototype.destroy = function() {
		this.dispatcher = null
	};
	StubRenderer.prototype.resize = function(width, height) {
		this.dispatcher.postMessage([0, width, height])
	};
	StubRenderer.prototype.render = function(y, cb, cr) {
		this.dispatcher.postMessage([1, y, cb, cr])
	};
	return StubRenderer
}();